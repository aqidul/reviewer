...
function generateReferralCode(int $user_id): string {
    ...
    $code = 'RF' . strtoupper(substr(md5((string)$user_id . time() . random_bytes(4)), 0, 6));
...
function getEmailTemplate(string $title, string $message, string $name): string {
    ...
    <head>
        ...
        <style>
            ...
            .button { display: inline-block; padding: 12px 30px; background: linear-gradient(135deg, #667eea, #764ba2); color: white; text-decoration: none; border-radius: 5px; margin-top: 20px; font-weight: 600; }
